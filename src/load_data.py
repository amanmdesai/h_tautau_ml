...
# main purpose is to be able to visualize the dataset 
...
def model(data):
    ...
    #print(X,y) 
    X = data[["DER_deltar_tau_lep","DER_mass_MMC","Weight","PRI_jet_subleading_pt","PRI_met_phi"]]
    X_train, X_test, y_train, y_test = train_test_split(X,y,random_state=33,test_size=0.25)
    scaler = preprocessing.StandardScaler().fit(X_train)
    X_train = scaler.transform(X_train)
    X_test = scaler.transform(X_test)
    assert X_train.shape[0] == y_train.shape[0]
    clf = GradientBoostingClassifier()#criterion='entropy',max_depth=3,min_samples_leaf=5)
